...
from .helpers import map_dict_to_device, map_target_to_device
...
#         # self.df_path = "reflacx_with_stay.csv"
...
#         # self.df: pd.DataFrame = pd.read_csv(
#         #     os.path.join(spreadsheets_folder, "reflacx_with_clinical.csv"), index_col=0
#         # )
...
class ReflacxObjectDetectionWithFixations(data.Dataset):
    """
    Class to load the preprocessed REFLACX master sheet. There `.csv` files are required to run this class.
    ...
    def prepare_input_from_data(
        ...
        images = [t["image"].to(device) for t in inputs]
        targets = [map_dict_to_device(t, device) for t in targets]
