...
class COCOeval:
    ...
    def summarize(self):
        ...
        def _summarize(ap=1, iouThr=None, areaRng="all", maxDets=100): 
            ...
            p_title_str = f" {titleStr:<18} {typeStr} @[ {io_type_str}={iouStr:<9} | area={areaRng:>6s} | maxDets={maxDets:>3d} ]"
        ...
        def _summarizeDets() -> list[dict]:
            ...
            # stats[1] = _summarize(1, maxDets=self.params.maxDets[0])
            # stats[2] = _summarize(1, maxDets=self.params.maxDets[1])
            # stats[3] = _summarize(1, maxDets=self.params.maxDets[2])
            # stats[4] = _summarize(1, maxDets=self.params.maxDets[3])
            # stats[5] = _summarize(1, maxDets=self.params.maxDets[4])
            # stats[6] = _summarize(1, iouThr=.5, maxDets=self.params.maxDets[1])
            # stats[7] = _summarize(1, iouThr=.75, maxDets=self.params.maxDets[2])
            ...
            # stats[11] = _summarize(0, maxDets=self.params.maxDets[0])
            # stats[12] = _summarize(0, maxDets=self.params.maxDets[1])
            # stats[13] = _summarize(0, maxDets=self.params.maxDets[2])
...
def get_eval_params_dict(
    ...
    # eval_params_dict["bbox"].areaRng = areaRng
...
def evaluate(coco_bbox_evaluator):
    """
    Run per image evaluation on given images and store results (a list of dict) in self.evalImgs
    :return: None
    """
    ...
    if p.useCats:
        p.catIds = list(np.unique(p.catIds))
    p.maxDets = sorted(p.maxDets)
    coco_bbox_evaluator.params = p

    coco_bbox_evaluator._prepare()
    # loop through images, area range, max detection number
    catIds = p.catIds if p.useCats else [-1]
    ...
    if p.iouType == "segm" or p.iouType == "bbox":
        ...
        computeIoU = get_compute_IoU_func(coco_bbox_evaluator)
    elif p.iouType == "keypoints":
        computeIoU = coco_bbox_evaluator.computeOks
    coco_bbox_evaluator.ious = {
        (imgId, catId): computeIoU(imgId, catId)
        for imgId in p.imgIds
        for catId in catIds
    }

    evaluateImg = coco_bbox_evaluator.evaluateImg
    maxDet = p.maxDets[-1]
    evalImgs = [
        evaluateImg(imgId, catId, areaRng, maxDet)
        for catId in catIds
    ...
    coco_bbox_evaluator._paramsEval = copy.deepcopy(coco_bbox_evaluator.params)
    # toc = time.time()
    # print('DONE (t={:0.2f}s).'.format(toc-tic))
...
def get_iobbs(d, g, iscrowd):
    ...
    for d_b in d:
        ...
        for g_b in g:
            ...
            d_iobbs.append(
                get_iobb(
                    pred_bb=get_input_coord_dict(d_b),
                    gt_bb=get_input_coord_dict(g_b),
                )
            )
